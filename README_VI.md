...
- `AUTO_ROLE_ON_JOIN_ENABLED` (mặc định `true`) là công tắc bật/tắt handler join mà không cần xóa Role ID. Đặt `false`/`0`/`off`/`no` (hoặc để trống) để tắt. `/syncrole` không bị ảnh hưởng bởi flag này.
...
- Lệnh `/syncrole` (slash + text): quét toàn bộ member hiện có, cấp role cho ai chưa có. Yêu cầu quyền `Manage Roles`. Reply ephemeral (chỉ người gọi thấy). Có sleep 0.5s giữa các lượt cấp để tránh rate limit. Log vào event `auto_role_sync`.
- Dashboard có panel **Tiến độ /syncrole** ở tab Tổng quan: progress bar + 3 chip (Cấp/Skip/Lỗi) + thời gian bắt đầu/kết thúc. Bot ghi `data/syncrole_progress.json` mỗi ~1s, dashboard poll mỗi 2s, panel tự ẩn nếu chưa từng chạy `/syncrole`. Hoàn tất sẽ bắn toast.
