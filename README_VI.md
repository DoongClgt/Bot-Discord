...
Giveaway dạng button lấy cảm hứng từ GiveawayBot / Carl-bot. Admin chạy `/giveaway start`, user bấm nút 🎉 để tham gia, bot tự kết thúc và random pick winner.

- `/giveaway start prize:<gì> duration:<10s|5m|1h|2d|1w> winners:<int> [description] [required_role] [channel]` (cần `Manage Server`). Duration min 10s, max 30 ngày.
- `/giveaway end <message_id>` — kết thúc sớm.
- `/giveaway reroll <message_id> [count]` — random lại winner cho giveaway đã end (giữ vĩnh viễn trong `data/giveaways.json`).
- `/giveaway cancel <message_id>` — hủy không pick winner; button disable, embed đổi "đã hủy".
- `/giveaway list` — list giveaway đang chạy trong server.
- Tham gia: bấm 🎉 để tham gia, bấm lại để rút. Nếu set `required_role`, user không đủ role sẽ thấy ephemeral cảnh báo. Button label hiện count realtime.
- Scheduler: `tasks.loop(seconds=30)` quét tìm giveaway hết hạn rồi end. Nếu bot down quá end time, tick đầu sau restart sẽ pickup hết.
- Khi end: bot edit embed gốc hiển thị winners, post tin reply mention winners, append record vào `data/giveaways_history.jsonl`. Winner chỉ được **mention trong channel** (không DM).
- State persist trong `data/giveaways.json` keyed by message_id. Persistent view (`custom_id="giveaway:enter"`) sống qua restart.
- Dashboard: tab **Giveaway** có form **Tạo giveaway** ở đầu trang (prize, duration, winners, dropdown channel, dropdown role, description) + 2 bảng active (kèm nút "End now") và 50 ended gần nhất (kèm nút "Reroll"). API: `GET /api/giveaways`, `POST /api/giveaways/start`, `POST /api/giveaways/end`, `POST /api/giveaways/reroll`. Action gọi qua IPC.
- Event log: `giveaway_start`, `giveaway_end`, `giveaway_reroll`, `giveaway_cancel`.
